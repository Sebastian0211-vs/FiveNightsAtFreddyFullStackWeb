...
import {
    fanAnimation, mainRoomDoorLeft, mainRoomDoorRight,
    tabletAnimation, noiseMenu, foxyRun, officeFreddy,
} from '../data/animations.js';
import { initMinimap } from '../camera/minimap.js';
import { initGameLogic, GameState } from '../engine/gameState.js';
import {
    playChicaJumpscare, playBonnieJumpscare, playFoxyJumpscare,
    playFreddyJumpscare, playGoldenFreddyJumpscare,
} from '../engine/jumpscare.js';

// ── Expose globals nécessaires au moteur de jeu ────────────────
// Le moteur accède à state, powerOut, renderPaused, ctx, W, H via window.*
// On les expose ici depuis le composant React
...
export default function MainRoom() {
    ...
    useEffect(() => {
        ...
        // ── Expose globals attendus par gameState.js ─────────────
        window.ctx          = ctx;
        window.renderPaused = false;
        Object.defineProperty(window, 'W', { get: () => W, configurable: true });
        Object.defineProperty(window, 'H', { get: () => H, configurable: true });

        // ── Navigation helper (utilisé par jumpscare GO_MENU) ─────
        window._reactNavigate = navigate;

        // --- Audio ---
        const audioBase = '../../assets/FNaF 1 Audio/';
        var sfxFan   = new Audio(audioBase + 'Buzz_Fan_Florescent2.wav');
        sfxFan.loop = true; sfxFan.volume = 0.2;
        var sfxPhone = new Audio(audioBase + 'voiceover1c.wav');
        sfxPhone.volume = 0.3;
        ...
        // Expose audio globals pour gameState.js
        window.sfxFan       = sfxFan;
        window.sfxPhone     = sfxPhone;
        ...
        function getActiveBg() {
            if (window.bonnieAtDoor) {
                const leftLit = state.left.light === 'on' && flickerOverride.left !== 'off';
                if (leftLit) {
                    if (!window._bonnieDoorImg) {
                        window._bonnieDoorImg = new Image();
                        window._bonnieDoorImg.src = '../../assets/Main Room/225.png';

                        const windowscaresfx = new Audio('../../assets/FNaF 1 Audio/windowscare.wav');
                        windowscaresfx.volume = 1;
                        window._bonnieDoorImg.onload = () => {
                            windowscaresfx.play().catch(() => {});
                        };
                    }
                    if (window._bonnieDoorImg.complete && window._bonnieDoorImg.naturalWidth) {
                        return window._bonnieDoorImg;
                    }
                }
            }
            if (window.chicaAtDoor) {
                const rightLit = state.right.light === 'on' && flickerOverride.right !== 'off';
                if (rightLit) {
                    if (!window._chicaDoorImg) {
                        window._chicaDoorImg = new Image();
                        window._chicaDoorImg.src = '../../assets/Main Room/227.png';

                        const windowscaresfx = new Audio('../../assets/FNaF 1 Audio/windowscare.wav');
                        windowscaresfx.volume = 1;
                        window._chicaDoorImg.onload = () => {
                            windowscaresfx.play().catch(() => {});
                        };
                    }
                    if (window._chicaDoorImg.complete && window._chicaDoorImg.naturalWidth) {
                        return window._chicaDoorImg;
                    }
                }
            }
        ...
        sfxDoor.volume = 0.8;
        var sfxLight = new Audio('../../assets/FNaF 1 Audio/BallastHumMedium2.wav');
        sfxLight.loop = true; sfxLight.volume = 0.6;

        // Expose pour gameState.js
        window.sfxLight = sfxLight;
        ...
        // Expose startDoorAnim for gameState.js
        window.startDoorAnim = startDoorAnim;
        ...
        const state = { left: { door: 'open', light: 'off' }, right: { door: 'open', light: 'off' } };
        ...
        // Expose renderPaused through window so gameState.js can set it
        Object.defineProperty(window, 'renderPaused', {
            get: () => renderPaused,
            set: (v) => { renderPaused = v; },
            configurable: true,
        });
        ...
        function draw() {
            ...
            if (!powerOut) {
                ...
                if (_officeFreddyAnim && _officeFreddyAnim.show) {
                    const img = officeFreddyFrames[_officeFreddyAnim.index];
        ...
        function animate() {
            ...
            if (_officeFreddyAnim) {
                const now = performance.now();
                const msPerFrame = 1000 / officeFreddy.fps;
                if (now - _officeFreddyAnim.lastTime >= msPerFrame) {
                    _officeFreddyAnim.lastTime = now;
                    _officeFreddyAnim.show = Math.floor(Math.random() * 10) === 1;
                    _officeFreddyAnim.index++;
                    if (_officeFreddyAnim.index >= officeFreddyFrames.length) {
                        if (_officeFreddyAnimSfx) {
                            _officeFreddyAnimSfx.pause();
                            _officeFreddyAnimSfx.currentTime = 0;
                            _officeFreddyAnimSfx = null;
                        }
                        _officeFreddyAnim = null;
                    }
                }
            }
            draw();
        ...
        // Jumpscare functions and GameState are defined in gamelogic.js

        // J = jumpscare aléatoire (dev helper)
        function onKeyDownJ(e) {
            if (e.key === 'j' || e.key === 'J') {
        ...
        window.addEventListener('keydown', onKeyDownJ);
        ...
        var sfxCameraLoop = new Audio('../../assets/FNaF 1 Audio/MiniDV_Tape_Eject_1.wav');
        sfxCameraUp.volume = 0.5; sfxCameraDown.volume = 0.5;
        sfxCameraLoop.loop = true; sfxCameraLoop.volume = 0.4;

        // Expose pour gameState.js
        window.sfxCameraLoop = sfxCameraLoop;
        ...
        // ── Cam selector ─────────────────────────────────────────
        const camLabelOverlay = document.getElementById('cam-label-overlay');

        const CAM_LABELS = {
            show_stage:       'Show Stage',
            dining_area:      'Dining Area',
            pirate_cove:      "Pirate's Cove",
            west_hall:        'West Hall',
            west_hall_corner: 'West Hall Corner',
            backstage:        'Backstage',
            east_hall:        'East Hall',
            east_hall_corner: 'East Hall Corner',
            kitchen:          'Kitchen',
            supply_closet:    'Supply Closet',
            restrooms:        'Restrooms',
        };

        // Exposée globalement pour que minimap puisse l'appeler
        function selectCam(room, camLabel) {
            window.activeCam = room;
            if (camLabelOverlay) {
                camLabelOverlay.textContent = CAM_LABELS[room] || room;
            }
            camPan = 0.5; camPanDir = 1;
            if (room === 'kitchen') {
                kitchenVideo.play().catch(() => {});
            } else {
                kitchenVideo.pause();
            }
        }
        ...
        function showCamSelector() {
            if (camLabelOverlay) camLabelOverlay.style.display = 'block';
            if (typeof CAMS === 'undefined' || !CAMS.length) return;
            const current = CAMS.find(c => c.room === window.activeCam) || CAMS[0];
            selectCam(current.room, current.label);
        }
        function hideCamSelector() {
            if (camLabelOverlay) camLabelOverlay.style.display = 'none';
        }
        ...
        // --- Camera source — now using static images via getCamImagePath ---
        ...
        // stopCamVideo / startCamVideo — drive the kitchen video element
        var camAudio = { pause() {}, play() { return Promise.resolve(); }, currentTime: 0 };
        window.camAudio = camAudio;
        const kitchenVideo = document.getElementById('kitchen-video');

        const officeFreddyFrames = officeFreddy.frames.map(src => {
            const img = new Image(); img.src = src; return img;
        });
        let _officeFreddyAnim = null;
        let _officeFreddyAnimSfx = null;

        if (!GameState._6amTriggered || !GameState._powerOutTriggered) {
            setInterval(() => {
                if (_officeFreddyAnim) return;
                if (window.isTabletOpen) return;
                if (renderPaused) return;
                if (Math.floor(Math.random() * 1000) !== 1) return;

                _officeFreddyAnimSfx = new Audio('../../assets/FNaF 1 Audio/robotvoice.wav');
                _officeFreddyAnimSfx.volume = 0.7;
                _officeFreddyAnimSfx.play().catch(() => {});

                _officeFreddyAnim = { index: 0, lastTime: performance.now(), show: false };
            }, 1000);
        }

        function startCamVideo() {
            if (window.activeCam === 'kitchen') {
                kitchenVideo.play().catch(() => {});
            }
        }
        function stopCamVideo() {
            kitchenVideo.pause();
            kitchenVideo.currentTime = 0;
        }
        // Expose stopCamVideo for gameState.js
        window.stopCamVideo = stopCamVideo;

        // Set initial cam to show_stage
        ...
        // ── Foxy run cam overlay ──────────────────────────────────────
        const foxyRunFrames = foxyRun.frames.map(src => {
            const img = new Image(); img.src = src; return img;
        });
        let foxyRunIndex = 0, foxyRunAccum = 0;
        const foxyRunMsPerFrame = 1000 / foxyRun.fps;
        ...
        function drawCamView() {
            ...
            const src = (typeof getCamImagePath !== 'undefined' && window.activeCam)
                ? getCamImagePath(window.activeCam)
                : null;

            if (window.activeCam === 'kitchen' && kitchenVideo.readyState >= 2) {
                // Draw video frame — cover-fill with subtle pan
                const vw = kitchenVideo.videoWidth || cw;
                const vh = kitchenVideo.videoHeight || ch;
                const scaleH = ch / vh, scaleW = cw / vw;
                const scale  = Math.max(scaleH, scaleW * 1.4);
                const maxOffset = Math.max(0, (vw * scale) - cw);
                const offsetX   = camPan * maxOffset;
                for (let sx = 0; sx < cw; sx++) {
                    ...
                    camCtx.drawImage(kitchenVideo, imgX, 0, 1, vh, sx, (ch - stripH) / 2, 1, stripH);
                }
            } else {
                const camImg = getOrLoadCamImg(src);
                if (camImg && camImg.complete && camImg.naturalWidth) {
                    // Cover-fill with subtle side pan
                    const imgW = camImg.naturalWidth, imgH = camImg.naturalHeight;
                    const scaleH = ch / imgH, scaleW = cw / imgW;
                    const scale  = Math.max(scaleH, scaleW * 1.4);
                    const maxOffset = Math.max(0, (imgW * scale) - cw);
                    const offsetX = camPan * maxOffset;
                    for (let sx = 0; sx < cw; sx++) {
                        const imgX = (offsetX + sx) / scale;
                        if (imgX < 0 || imgX >= imgW) continue;
            ...
            // ── Foxy run frame drawn BEFORE noise so noise composites on top ─
            if (window.foxyRunning && window.activeCam === 'west_hall' && !window.foxyRunAnimDone) {
                const fFrame = foxyRunFrames[foxyRunIndex];
                if (fFrame && fFrame.complete && fFrame.naturalWidth) {
                    const sc = Math.max(cw / fFrame.naturalWidth, ch / fFrame.naturalHeight);
                    const dw = fFrame.naturalWidth * sc, dh = fFrame.naturalHeight * sc;
                    camCtx.drawImage(fFrame, (cw - dw) / 2, (ch - dh) / 2, dw, dh);
                }
            }

            let noiseB = Math.floor(Math.random() * 3);
            setInterval(() => { noiseB = Math.floor(Math.random() * 3); }, 1000);
            ...
            if (noiseFrame && noiseFrame.complete && noiseFrame.naturalWidth) {
                ...
                camCtx.globalAlpha = ((150 + Math.random() * 50 + noiseB * 15) / 245)*0.7;
            ...
            // ── Foxy run trigger logic (sfx + 3s timer) on cam 2A ────────────
            if (window.foxyRunning && window.activeCam === 'west_hall') {
                if (!window._foxyRunCamSfxTriggered) {
                    window._foxyRunCamSfxTriggered = true;
                    if (typeof foxy !== 'undefined') foxy.onWatchRunCam();
                }
            } else if (!window.foxyRunning) {
                window._foxyRunCamSfxTriggered = false;
                if (foxyRunIndex !== 0) { foxyRunIndex = 0; foxyRunAccum = 0; }
            }
        ...
        function tickNoise(timestamp) {
            ...
            // Advance foxy run frame independently — plays once, stops at last frame
            if (!window.foxyRunAnimDone) {
                foxyRunAccum += delta;
                while (foxyRunAccum >= foxyRunMsPerFrame) {
                    foxyRunAccum -= foxyRunMsPerFrame;
                    if (foxyRunIndex < foxyRunFrames.length - 1) {
                        foxyRunIndex++;
                    } else {
                        window.foxyRunAnimDone = true;
                        break;
                    }
                }
            }
            ...
            if (timestamp - lastFrameTime > tabletAnimation.frameDelay) {
                ...
                if (tabletState === 'opening') {
                    ...
                    if (tabletFrame < 0) {
                        ...
                        if (typeof bonnie !== 'undefined') bonnie.onTabletClose();
                        if (typeof chica !== 'undefined') chica.onTabletClose();
                        window.hideMinimap && window.hideMinimap();
                        return;
                    }
                    drawTabletFrame(tabletFrame);
                }
        ...
        function openTablet() {
            ...
            if (typeof bonnie !== 'undefined') bonnie.onTabletOpen();
            if (typeof chica !== 'undefined') chica.onTabletOpen();
        ...
        function closeTablet() {
            ...
            if (typeof foxy !== 'undefined') foxy.onTabletClose();
            //if (typeof bonnie !== 'undefined') bonnie.onTabletClose();
        ...
        window.addEventListener('mousemove', e => {
            ...
            const onMinimap = minimap && minimap.matches(':hover');
            if (fromBottom < 60 && !onMinimap && tabletState === 'closed') openTablet();
            if (fromBottom < 60 && !onMinimap && tabletState === 'open')   closeTablet();
        });
        ...
        requestAnimationFrame(() => requestAnimationFrame(() => { if (fadeEl) fadeEl.style.opacity = '0'; }));

        // ── Start game logic ──────────────────────────────────────────
        // GameState, jumpscares, and constants are all defined in gamelogic.js
        initGameLogic();
        ...
        return () => {
            ...
            window.removeEventListener('keydown', onKeyDownJ);
            tabletBar.removeEventListener('click', openTablet);
        };
    }, [navigate]);
        ...
        <>
            ...
            <div id="hud-top-right" style={{
                ...
                <div id="hud-time" style={{
                    fontSize: 'calc(clamp(24px, 3.6vw, 56px) * var(--hud-tr-scale, 1.6))',
                    letterSpacing: '0.06em',
                    lineHeight: 1,
                }}>12 AM</div>
                <div id="hud-night" style={{
                    fontSize: 'calc(clamp(13px, 1.6vw, 26px) * var(--hud-tr-scale, 1.6))',
            ...
            <div id="hud-power" style={{
                ...
                color: '#fff',
                textShadow: '0 0 10px rgba(255,255,255,0.5)',
                fontSize: 'calc(clamp(13px, 1.6vw, 26px) * var(--hud-bl-scale, 2.25))',
                letterSpacing: '0.08em',
            }}>
                Power Left: <span id="hud-power-val">99%</span>
            </div>

            {/* HUD usage: label + battery image */}
            <div id="hud-usage" style={{
                position: 'fixed',
                ...
                <span>Usage :</span>
                <img id="hud-battery-img"
                    src="../../assets/Battery/212.png"
                    alt="battery"
                    style={{ height: 'clamp(28px, 4vw, 52px)', width: 'auto', imageRendering: 'pixelated' }}
                />
            </div>

            <div id="tablet-bar" style={{
                position: 'fixed', bottom: 0, left: 0, width: '100%', height: '40px',
                background: 'transparent', borderTop: 'none', zIndex: 550,
            ...
            {/* Cam label — visible quand tablette ouverte */}
            <div id="cam-label-overlay" style={{
                display: 'none',
                position: 'fixed',
                bottom: 'calc(2vh + 260px)',
                right: '1vw',
                left: 'auto',
                top: 'auto',
                zIndex: 200,
            ...
            <canvas id="tablet-anim-canvas" style={{
                display: 'none', position: 'fixed', inset: 0, width: '100%', height: '100%', zIndex: 45,
            }} />
            <canvas id="tablet-cam" style={{
                display: 'none', position: 'fixed', inset: 0, zIndex: 44,
                background: '#000', width: '100%', height: '100%',
            }} />
            {/* Hidden video element for kitchen cam */}
            <video id="kitchen-video" src=""
                loop playsInline
