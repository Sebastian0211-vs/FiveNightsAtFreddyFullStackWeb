// ============================================================
//  gameState.js  —  Game state, power, animatronic AI, jumpscares
//
//  Depends on:  animations.js  (animation defs)
//  Expects these refs injected via GameState.init():
//    _state            — door/light state object  { left, right }
//    _powerOutSetter   — function(bool)
//    _renderPausedSetter — function(bool)
//    _getCtx / _getW / _getH — canvas getters
//    _sfxFan, _sfxPhone, _sfxLight, _sfxCameraLoop, _camAudio
//    _stopCamVideo()
//    _startDoorAnim(side, direction)
//    _jumpscareHandlers
//    _navigate
//    _hudSetter(hudObj)
//    _hudVisibilitySetter(bool)
//    _animatronics
//    _foxy, _bonnie
// ============================================================

import { HOURS, NIGHT_SECS, SECS_PER_HOUR, PASSIVE_INTERVAL, base_ai_level, ANIM_INTERVALS } from '../constants/nightConfig.js';
...
// ── Camera image cache ────────────────────────────────────────
export const _camCache = {};

function _pick(arr) { return arr[Math.floor(Math.random() * arr.length)]; }

function _stablePick(room, arr) {
...
export function getCamImagePath(room, foxy_ref) {
...
export const GameState = {
    ...
    // Injected references (set via GameState.init())
    _state:              null,
    _powerOut:           false,
    _powerOutSetter:     null,
    _renderPausedSetter: null,
    _getCtx:             null,
    _getW:               null,
    _getH:               null,
    _sfxFan:             null,
    ...
    _stopCamVideo:       null,
    _startDoorAnim:      null,
    _jumpscareHandlers:  null,
    _navigate:           null,
    _hudSetter:          null,
    _hudVisibilitySetter: null,
    _animatronics:       null,
    _foxy:               null,
    _bonnie:             null,
    ...
    onPowerOut() {
        ...
        this._powerOut = true;
        if (this._powerOutSetter) this._powerOutSetter(true);
        window._powerOutEyeFrame = '304';

        if (this._hudVisibilitySetter) this._hudVisibilitySetter(false);

        [this._sfxFan, this._sfxPhone, this._sfxLight, this._sfxCameraLoop, this._camAudio].forEach(a => {
            if (!a) return;
            a.pause();
            a.currentTime = 0;
        });
        if (this._stopCamVideo) this._stopCamVideo();
        ...
        if (state) {
            ['left', 'right'].forEach(side => {
                ...
                if (this._startDoorAnim) this._startDoorAnim(side, -1);
            });
        }
                        ...
                        this._powerOutTimers.push(setTimeout(() => {
                            ...
                            this._powerOutTimers.push(setTimeout(() => {
                                ...
                                steps4.addEventListener('ended', () => {
                                    ...
                                    if (this._jumpscareHandlers) this._jumpscareHandlers.playPowerOutJumpscare();
                                });
                            }, 500));
    ...
    on6AM() {
        ...
        if (this._powerOutSetter) this._powerOutSetter(false);
        ...
        if (this._renderPausedSetter) this._renderPausedSetter(true);
                    ...
                    setTimeout(() => {
                        runPhase(FADEOUT_MS, 1, 0, H, H, () => {
                            if (this.night >= 6) {
                                ctx.fillStyle = '#000';
                                ctx.fillRect(0, 0, W, H);
                                setTimeout(() => { if (this._navigate) this._navigate('/menu'); }, 1000);
                                return;
                            }
                            this.night++;
                            this.rawPower       = 999;
                            ...
                            const animatronics = this._animatronics;
                            if (animatronics) {
                            ...
                            if (this._renderPausedSetter) this._renderPausedSetter(false);
                            this.renderHUD();
                            if (this._hudVisibilitySetter) this._hudVisibilitySetter(true);
                            if (this._sfxFan) this._sfxFan.play().catch(() => {});
                        });
                    }, HOLD2_MS);
                });
            }, HOLD_MS);
...
// ── Ambient audio ─────────────────────────────────────────────
export const _foxyPirateAudio = new Audio('./assets/FNaF 1 Audio/pirate song2.wav');
export const _circusAudio     = new Audio('./assets/FNaF 1 Audio/circus.wav');
export const _kitchenAudio1   = new Audio('./assets/FNaF 1 Audio/OVEN-DRA_1_GEN-HDF18119.wav');
export const _kitchenAudio2   = new Audio('./assets/FNaF 1 Audio/OVEN-DRA_2_GEN-HDF18120.wav');
export const _kitchenAudio3   = new Audio('./assets/FNaF 1 Audio/OVEN-DRA_7_GEN-HDF18121.wav');
export const _kitchenAudio4   = new Audio('./assets/FNaF 1 Audio/OVEN-DRAWE_GEN-HDF18122.wav');
    ...
    setInterval(() => {
        if (ROOMS['kitchen'].who.includes('Chica') && (!GameState._6amTriggered || !GameState._powerOutTriggered)) {
            ...
            switch (_chicaKitchenSound) {
                case 1: _kitchenAudio1.currentTime = 0; _kitchenAudio1.play().catch(() => {});
                    _kitchenAudio1.volume = window.activeCam === 'kitchen' ? 0.6 : 0.2; return;
                case 2: _kitchenAudio2.currentTime = 0; _kitchenAudio2.play().catch(() => {});
                    _kitchenAudio2.volume = window.activeCam === 'kitchen' ? 0.6 : 0.2; return;
                case 3: _kitchenAudio3.currentTime = 0; _kitchenAudio3.play().catch(() => {});
                    _kitchenAudio3.volume = window.activeCam === 'kitchen' ? 0.6 : 0.2; return;
                case 4: _kitchenAudio4.currentTime = 0; _kitchenAudio4.play().catch(() => {});
                    _kitchenAudio4.volume = window.activeCam === 'kitchen' ? 0.6 : 0.2; return;
                default: return null;
            }
        }
    }, 4000);
